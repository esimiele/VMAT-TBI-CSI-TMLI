...
namespace AutoPlannerHelpers.Helpers
{
    public static class OptimizationSetupHelper
    {
        ...
        //    if (!currentList.Any()) currentList = RetrieveOptConstraintsFromTemplate(selectedTemplate, prescriptions);
        //    if (currentList.Any())
        //    {
        ...
        //    {
        //        if (jnxs.Any(x => string.Equals(x.PlanId, itr.PlanId, StringComparison.OrdinalIgnoreCase)))
        //        {
        ...
        //            foreach (Structure itr1 in jnxs.First(x => string.Equals(x.PlanId, itr.PlanId, StringComparison.OrdinalIgnoreCase)).FieldJunctions.Select(x => x.JunctionStructure))
        //            {
        ...
        /// <param name="prescriptions"></param>
        /// <returns></returns>
        public static List<PlanOptimizationSetupModel> RetrieveOptConstraintsFromTemplate(AutoPlanTemplateBase selectedTemplate, List<PrescriptionModel> prescriptions, PlanType type)
        {
        ...
        /// <returns></returns>
        public static List<PlanOptimizationSetupModel> CreateOptimizationConstraintList(AutoPlanTemplateBase selectedTemplate, Dictionary<string, string> planTargets, PlanType type)
        {
            List<PlanOptimizationSetupModel> list = new List<PlanOptimizationSetupModel> { };
            //no treatment template selected => scale optimization objectives by ratio of entered Rx dose to closest template treatment Rx dose
            if (!ReferenceEquals(selectedTemplate, null))
