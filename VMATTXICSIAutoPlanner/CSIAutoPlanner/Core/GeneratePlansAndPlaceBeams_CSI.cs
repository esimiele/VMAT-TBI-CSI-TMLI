...
namespace CSIAutoPlanner.Core
{
    internal class GeneratePlansAndPlaceBeams_CSI : BeamPlacementBase
    {
        ...
        internal GeneratePlansAndPlaceBeams_CSI(List<PlanIsocenterModel> planInfo,
                                                ...
                                                double overlapMargin)
        {
            ...
            courseId = CSIAutoPlannerSettings.CourseId;
            calculationModel = CSIAutoPlannerSettings.DoseCalculationAlgorithm;
            optimizationModel = CSIAutoPlannerSettings.OptimizationAlorithm;
            useGPUdose = CSIAutoPlannerSettings.UseGPUForDosecalculation;
            useGPUoptimization = CSIAutoPlannerSettings.UseGPUForOptimization;
            MRrestart = CSIAutoPlannerSettings.MRLevelRestart;
        ...
        private bool ExtendBodyContour()
        {
            ...
            if (EclipseContext.GetInstance().StructureSet.CanAddStructure("CONTROL", "human_body"))
            {
                ...
                Structure bodyCopy = EclipseContext.GetInstance().StructureSet.AddStructure("CONTROL", "human_body");
        ...
        protected override List<PlanIsocenterModel> GetIsocenterPositions()
        {
            ...
            foreach (PlanIsocenterModel itr in planIsocenters)
            {
                ...
                (bool fail, Structure longestTargetInPlan, double maxTargetLength, StringBuilder errorMessage) = TargetsHelper.GetLongestTargetInPlan(planIdTargets.FirstOrDefault(x => string.Equals(x.PlanId, itr.PlanId, StringComparison.OrdinalIgnoreCase)), EclipseContext.GetInstance().StructureSet);
                if (fail)
                {
        ...
        protected override bool SetVMATBeams(PlanIsocenterModel planIso)
        {
            ...
            List<PlanTargetsModel> planIdTargets = new List<PlanTargetsModel>(TargetsHelper.GetTargetListForEachPlan(prescriptions));
            ProvideUIUpdate("Determining target with greatest extent");
            (bool fail, Structure longestTargetInPlan, double maxTargetLength, StringBuilder errorMessage) = TargetsHelper.GetLongestTargetInPlan(planIdTargets.FirstOrDefault(x => string.Equals(x.PlanId, planIso.PlanId, StringComparison.OrdinalIgnoreCase)), EclipseContext.GetInstance().StructureSet);
            if (fail)
            {
                ProvideUIUpdate($"Error! No structure named: {errorMessage} found or contoured!", true);
            }
            ...
            foreach (IsocenterModel itr in planIso.Isocenters)
            {
                ...
                for (int j = 0; j < itr.NumberOfBeams; j++)
                {
                    ...
                    double coll = CSIAutoPlannerSettings.CollimatorRotations[j];
        ...
        private VVector[] GetLateralStructureBoundingBox(Structure target, double zMin, double zMax)
        {
            ...
            int startSlice = CalculationHelper.ComputeSlice(zMin, EclipseContext.GetInstance().StructureSet.Image.Origin.z, EclipseContext.GetInstance().StructureSet.Image.ZRes);
            //only go to the most superior part of the lungs for contouring the arms
            int stopSlice = CalculationHelper.ComputeSlice(zMax, EclipseContext.GetInstance().StructureSet.Image.Origin.z, EclipseContext.GetInstance().StructureSet.Image.ZRes);
            int percentComplete = 0;
