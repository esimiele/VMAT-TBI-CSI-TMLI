...
namespace TMLIAutoPlanner.Core
{
    internal class GeneratePlansAndPlaceBeams_TMLI : GeneratePlansAndPlaceBeamsBase
    {
        ...
        public GeneratePlansAndPlaceBeams_TMLI() { }
        ...
        public GeneratePlansAndPlaceBeams_TMLI(List<PlanIsocenterModel> planInfo,
        ...
        protected override List<PlanIsocenterModel> GetIsocenterPositions()
        {
            ...
            if (!TMLIAutoPlannerSettings.AllBeamsVMAT && StructureTuningHelper.DoesStructureExistInSS("matchline", true))
            {
                ...
                allIsocenters.Add(new PlanIsocenterModel(vmatPlan.Id, CalculateVMATIsoPositions(targetSupExtent,
                                                                                                ...
                                                                                                10.0,
                                                                                                TMLIAutoPlannerSettings.MaxFieldYExtent,
                                                                                                TMLIAutoPlannerSettings.MinFieldOverlap,
                                                                                                offsetY,
            ...
            if ((firstIso.z + TMLIAutoPlannerSettings.MaxFieldYExtent / 2 - targetSupExtent < checkIsoPlacementLimit) ||
                (targetInfExtent - (lastIso.z - TMLIAutoPlannerSettings.MaxFieldYExtent / 2) < checkIsoPlacementLimit)) checkIsoPlacement = true;
        ...
        protected override bool SetVMATBeams(PlanIsocenterModel planIso)
        {
            ...
            foreach (IsocenterModel itr in planIso.Isocenters)
            {
                ...
                for (int j = 0; j < itr.NumberOfBeams; j++)
                {
                    ...
                    VRect<double> jp = TMLIAutoPlannerSettings.JawPositions.ElementAt(j);
                    if ((totalNumIsos > numVMATIsos) && (isoCount == (numVMATIsos - 1)))
                    {
                        ...
                        else
                        {
                            ...
                            if (j == 2)
                            {
                                jp = TMLIAutoPlannerSettings.JawPositions.ElementAt(j + 1);
                            }
                            else if (j == 3)
                            {
